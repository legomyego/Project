...
public static class RecipeEndpoints
{
    ...
    public static void MapRecipeEndpoints(this WebApplication app)
    {
        ...
        group.MapPost("/", CreateRecipeAsync)
            ...
            .WithDescription("Create a new recipe. Only authenticated users can create recipes.")
            .RequireAuthorization(); // Requires authentication
        ...
        AppDbContext db)
    {
        ...
        // Check if recipe requires a subscription
        if (recipe.RequiresSubscription)
        {
            ...
            {
                // User is not authenticated - cannot access subscription recipe
                return Results.Unauthorized();
            }

            // Check if user has an active subscription that includes this recipe
            var hasAccess = await db.UserSubscriptions
                .Where(us => us.UserId == userId
                    && us.IsActive
                    && us.EndDate > DateTime.UtcNow)
                .Join(db.SubscriptionRecipes,
                    us => us.SubscriptionId,
                    sr => sr.SubscriptionId,
                    (us, sr) => new { us, sr })
                .AnyAsync(x => x.sr.RecipeId == id);

            if (!hasAccess)
            {
                // User doesn't have an active subscription that includes this recipe
                return Results.Json(
                    new { error = "This recipe requires an active subscription" },
                    statusCode: 403
                );
            }
        }
    ...
    /// Contains all required information for a recipe
    /// </summary>
    ...
    /// <summary>
    /// Create a new recipe
    /// Authenticated users can create recipes that will be added to the catalog
    /// The creator automatically becomes the author
    /// </summary>
        ...
        AppDbContext db)
    {
        ...
        // Load author information for response
        await db.Entry(recipe).Reference(r => r.Author).LoadAsync();

        // Return created recipe with author details
